...
class Parser {
	 ...
	 *
	 * This is deliberately *not* WordPress's remove_accents(). Owning the table
	 * keeps this class free of WordPress, which is what lets the fixture suite
	 * run standalone. A function_exists() branch would be worse than no folding
	 * at all: the tests would exercise the fallback while production exercised
	 * remove_accents(), so the shipping path would never be the tested path and
	 ...
	 */
	private const ASCII_FOLD = array(
		// Latin-1 Supplement.
	...
	/**
	 * Normalised key used to spot the same product listed twice.
	 *
	...
	private function duplicate_key( string $name, string $variant ): string {
		...
		return strtolower( (string) preg_replace( '/[^a-z0-9]/iu', '', $folded ) );
	}
	...
	private function flag_duplicates( array $rows ): array {
		...
		foreach ( $rows as $index => $row ) {
			...
			$key = $this->duplicate_key( (string) $row['name'], (string) $row['variant'] );
